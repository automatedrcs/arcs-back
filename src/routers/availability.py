...
from database.schema import AvailabilityCreate, AvailabilityUpdate, Availability  # Assuming you have an AvailabilityUpdate schema
from database.models import Availability as AvailabilityModel  # assuming models.Availability is the ORM model
...
def get_availabilities(
    ...
    query = query.filter(AvailabilityModel.organization_id == organization_id)
...
@availability_router.get("/", response_model=List[Availability])
def read_availabilities(
    ...
    return get_availabilities(db, organization_id, skip=skip, limit=limit, person_id=person_id)
