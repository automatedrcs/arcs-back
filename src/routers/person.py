...
def create_person(db: Session, person: schema.PersonCreate) -> models.PersonModel:
    db_person = models.PersonModel(**person.dict())
    db.add(db_person)
    db.commit()
...
def get_people(
        ...
        skip: int = 0,
        limit: int = 100
) -> Union[models.PersonModel, List[models.PersonModel]]:
    query = db.query(models.PersonModel)

    if person_id:
        return query.filter(models.PersonModel.id == person_id).first()

    if organization_id:
        query = query.filter(models.PersonModel.organization_id == organization_id)
        
    if user_id:
        query = query.filter(models.PersonModel.user_id == user_id)

    if role:
        query = query.filter(models.PersonModel.role == role)

    return query.offset(skip).limit(limit).all()

def update_person(db: Session, person_id: UUID, person: schema.PersonUpdate) -> Optional[models.PersonModel]:
    db_person = db.query(models.PersonModel).filter(models.PersonModel.id == person_id).first()
    if db_person:
...
def delete_person(db: Session, person_id: UUID) -> Optional[models.PersonModel]:
    db_person = db.query(models.PersonModel).filter(models.PersonModel.id == person_id).first()
    if db_person:
        db.delete(db_person)
        db.commit()
...
def create_new_person(person: schema.PersonCreate, db: Session = Depends(database.get_db)) -> models.PersonModel:
